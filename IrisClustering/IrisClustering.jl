...
function plot_data(cluster_column::Symbol, ic_model::IrisModel)
  ...
  for species in Array(data[:, cluster_column]) |> unique!
    for r in eachrow(data[data[!, cluster_column] .== species, :])
      push!.((x_feature_collection, y_feature_collection),
            ((r[Symbol(ic_model.xfeature[])]), (r[Symbol(ic_model.yfeature[])])))
    end
    plot = PlotData(
    ...
    empty!(x_feature_collection)
    empty!(y_feature_collection)
  end
  plot_collection
end
